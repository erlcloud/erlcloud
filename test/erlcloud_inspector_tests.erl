...
add_attributes_to_findings_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"attributes">>, Attributes},
...
attach_assessment_and_rules_package_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"assessmentArn">>, ?ASSESSMENT_ARN},
...
create_application_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"applicationName">>, ?APPLICATION_NAME},
...
create_assessment_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"applicationArn">>, ?APPLICATION_ARN},
...
create_resource_group_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"resourceGroupTags">>, erlcloud_json:encode(?RESOURCE_GROUP_TAGS)}]),
...
delete_run_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"runArn">>, ?RUN_ARN}]),
...
describe_finding_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"findingArn">>, ?FINDING_ARN}]),
    Response = [{<<"finding">>,
                 [{<<"agentId">>, ?INSTANCE_ID},
                  {<<"attributes">>,
...
describe_resource_group_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"resourceGroupArn">>, ?RESOURCE_GROUP_ARN}]),
    Response = [{<<"resourceGroup">>,
                 [{<<"resourceGroupArn">>, ?RESOURCE_GROUP_ARN},
                  {<<"resourceGroupTags">>, erlcloud_json:encode(?RESOURCE_GROUP_TAGS)}]}],
    all_tests(Action, Function, PostData, Response).
...
describe_rules_package_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"rulesPackageArn">>, ?RULES_PACKAGE_ARN}]),
...
list_tags_for_resource_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"resourceArn">>, ?ASSESSMENT_ARN}]),
    Response = [{<<"tagList">>, []}],
    all_tests(Action, Function, PostData, Response).
...
localize_text_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"locale">>, <<"en_US">>},
...
register_cross_account_access_role_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"roleArn">>, ?ROLE_ARN}]),
...
remove_attributes_from_findings_tests(_) ->
    ...
    PostData = erlcloud_json:encode([{<<"attributeKeys">>, AttributeKeys},
                           {<<"findingArns">>, [?FINDING_ARN]}]),
    Response = [{<<"message">>, <<"Success">>}],
    all_tests(Action, Function, PostData, Response).
