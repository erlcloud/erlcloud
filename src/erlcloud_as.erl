...
%% @end
%% --------------------------------------------------------------------
-spec set_desired_capacity(string(), integer(), boolean(), aws_config()) ->
...
terminate_instance(InstanceId, false, Config) ->
    priv_terminate_instance([{"InstanceId", InstanceId},
                             {"ShouldDecrementDesiredCapacity", "false"}], Config);
terminate_instance(InstanceId, true, Config) ->
    priv_terminate_instance([{"InstanceId", InstanceId},
                             {"ShouldDecrementDesiredCapacity", "true"}], Config).
...
-spec suspend_processes(string(), list(string()), aws_config()) ->
                                    {ok, string()} | {error, term()}.
suspend_processes(GroupName, ScalingProcesses, Config) ->
    Params = [{"AutoScalingGroupName", GroupName}
              | member_params("ScalingProcesses.member.", ScalingProcesses)],
...
extract_as_activity(A) ->
    #aws_autoscaling_activity{
       id = erlcloud_xml:get_text("ActivityId", A),
       group_name = erlcloud_xml:get_text("AutoScalingGroupName", A),
       cause = erlcloud_xml:get_text("Cause", A),
       description = erlcloud_xml:get_text("Description", A),
       details = erlcloud_xml:get_text("Details", A),
       status_code = erlcloud_xml:get_text("StatusCode", A),
       status_msg = erlcloud_xml:get_text("StatusMessage", A),
...
-spec as_query(aws_config(), string(), list({string(), term()}), string()) ->
                                    {ok, term()} | {error, term()}.
